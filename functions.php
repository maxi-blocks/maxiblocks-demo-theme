...
function patterns_custom_taxonomies()
{
    ...
    register_taxonomy("light_or_dark", "post", [
    ...
    // register_taxonomy("pattern_base", "post", [
    //     // Hierarchical taxonomy (like categories)
    //     "hierarchical" => true,
    //     "show_in_rest" => true,
    //     "show_ui" => true,
...
add_action("init", "patterns_custom_taxonomies", 11);
...
{
    add_meta_box(
        "link-to-related-meta-box-id",
        esc_html__("ID of related tone variant", "text-domain"),
        "link_to_related_meta_box_callback",
        "post",
        "side",
        "high"
    );
...
{
    add_meta_box(
        "maxi-version-meta-box-id",
        esc_html__("Maxi version", "text-domain"),
        "maxi_version_meta_box_callback",
        "post",
        "side",
        "high"
    );
...
{
    $outline =
        '<label for="link_to_related">' .
        esc_html__(
            "The ID of related pattern with the opposite tone - light or dark",
            "text-domain"
        ) .
        "</label>";
    ...
    $outline .=
        '<input type="text" name="link_to_related" id="link_to_related" class="link_to_related" value="' .
        esc_attr($link_to_related) .
        '"/>';
...
{
    $outline =
        '<label for="maxi_version">' .
        esc_html__(
            "Maxi plugin version the item was created in",
            "text-domain"
        ) .
        "</label>";
    ...
    $outline .=
        '<input type="text" name="maxi_version" id="maxi_version" class="maxi_version" value="' .
        esc_attr($maxi_version) .
        '"/>';
...
function save_link_to_related($post_id)
{
    ...
    if (
        !wp_verify_nonce(
            $_POST["link_to_related_nonce"],
            "save_link_to_related"
        )
    ) {
        return $post_id;
    }
...
function maxi_register_post_meta_maxi_version()
{
    ...
    $maxi_plugin_info = get_plugin_data(
        WP_PLUGIN_DIR . "/maxi-blocks/plugin.php"
    );
...
function enqueue_roboto_font()
{
    wp_enqueue_style(
        "roboto-font",
        "https://fonts.bunny.net/css?family=roboto:300,400,500,700&display=swap"
    );
}
